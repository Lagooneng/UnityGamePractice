...
public class zFoxVirtualPad : MonoBehaviour
{
    ...
    {
        // 초기화
        uicam = GameObject.Find("FUIPadCamera").
    ...
    void RunAutoLayout()
    {
        ...
        if( autoLayout )
        {
            Vector3 scPos = uicam.ScreenToWorldPoint(
                new Vector3(Screen.width, Screen.height, 0.0f) - uicam.transform.position);
            Vector3 posPad = new Vector3(
                -scPos.x * autoLayoutPOS_SlidePad.x, -scPos.y * autoLayoutPOS_SlidePad.y, 0.0f);
            sprSlidePadBack.transform.localPosition = posPad;
            Vector3 posBtnA = new Vector3(
                scPos.x * autoLayoutPOS_ButtonA.x, -scPos.y * autoLayoutPOS_ButtonA.y, 0.0f);
            sprButtonA.transform.localPosition = posBtnA;
            Vector3 posBtnB = new Vector3(
                scPos.x * autoLayoutPOS_ButtonB.x, -scPos.y * autoLayoutPOS_ButtonB.y, 0.0f);
    ...
    private void Update()
    {
        ...
        switch( padValMode )
        {
            ...
            case zFOXVPAD_SLIDEPADVALUEMODE.PAD_XY_SCREEN_WW:
                ...
                break;
            case zFOXVPAD_SLIDEPADVALUEMODE.PAD_XX_SCREEN_HH:
    ...
    bool CheckSlidePadMove(Vector2 posToush)
    {
        if( movPadEnable )
        {
            movEnabled = true;
